 ...
 function pozovi(stranica){
                  ...
                  ajax.onreadystatechange = function () {
                    ...
                    if (ajax.readyState == 4 && ajax.status == 404)
                        document.getElementById("sredina").innerHTML = stranica.toString();
                }
                if(stranica == 'novosti'){
...
function createTable() {
    ...
    ajax.onreadystatechange = function () {
        ...
        if (ajax.readyState == 4 && ajax.status == 404) {
            document.getElementById('dokumenti').innerHTML = '<p>Tabela nije kreirana</p>';
        }

    }
    ajax.open("GET", 'http://zamger.etf.unsa.ba/wt/proizvodi.php?brindexa=16039', true);
    ajax.send();
    return false;
}

function create(tb) {
    var t = "<table class='tabela'>
            <tr><td>
                <input type='button' onclick=pomocna('dodavanje')  value='Dodaj proizvod'> </td><td> 
    ...
    t += '</table>';
    document.getElementById('dokumenti').innerHTML = t;
}
...
function pomocna(ver) {
    ...
    ajax.send("pomocna=" + ver + "&brindexa=16039&proizvod=" + JSON.stringify(temp));
}
